...
:::note
React Native iOS uses the **same native iOS SDK** (SharpsellCore + local XCFrameworks). Apply these steps on the `.xcworkspace` generated for the iOS app. There is no separate React Native iOS npm package. Call `Sharpsell.services` from AppDelegate / native iOS code.
:::
